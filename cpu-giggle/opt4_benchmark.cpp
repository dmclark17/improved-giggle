...
inline void opt4CPU_unpackC(GemmRun<float>* run, unsigned int j, unsigned int i, const float* c_pack) {
    /*
...
inline void opt4CPU_aux_simd_cleanup(float* a_pack, float* b_pack, float* c_pack) {
    __m256 a0;
    __m256 b0;
    __m256 c0, c1, c2, c3;
    __m128 sum0, sum1, sum2, sum3, sum4, sum5, sum6, sum7;
...
inline void opt4CPU_gepb(GemmRun<float>* run, unsigned int p, unsigned int j, float* a_pack, float* b_pack, float* c_pack) {
    ...
    */
    ...
    for (i = 0; i < run->m; i += MR) {
        ...
        opt4CPU_aux_simd_cleanup(a_pack + (i * KC), b_pack, c_pack);
